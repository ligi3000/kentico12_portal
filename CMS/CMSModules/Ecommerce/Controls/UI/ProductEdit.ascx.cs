    ...
    private void InitSkuForms()
    {
        ...
        skuGeneralForm.OnBeforeSave += (sender, args) =>
        {
            ...
            if (taxID == AUTOMATIC_TAX)
            {
                ...
                var defaultTaxClassID = DepartmentInfoProvider.GetDepartmentInfo(departmentID)?.DepartmentDefaultTaxClassID;
    ...
    private void InitPage()
    {
        ...
        if (page != null)
        {
            ...
            if (page.RequiresDialog)
            {
                ...
                if (treeNode != null)
                {
                    string nodeName = HTMLHelper.HTMLEncode(ResHelper.LocalizeString(treeNode.GetDocumentName()));
