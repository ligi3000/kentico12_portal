...
public partial class CMSWebParts_Events_EventManager_2 : CMSAbstractWebPart
{
    ...
    {
        if (StopProcessing || CMS.DocumentEngine.DocumentContext.CurrentDocument == null || CMS.DocumentEngine.DocumentContext.CurrentDocument.NodeClassName.ToLowerCSafe() != "cms.bookingevent")
        {
            // Do nothing
            Visible = false;
        }
        else
        {
            ...
            EventNode = CMS.DocumentEngine.DocumentContext.CurrentDocument;
    ...
    protected void DisplayRegistration()
    {
        ...
        var auth = AuthenticationHelper.IsAuthenticated();

        // Display registration form to anonymous user only if this is allowed
        if ((AllowAnonymousRegistration || (userInfo != null && auth)) && EventNode != null)
        {
            ...
            if ((openFrom == DateTimeHelper.ZERO_TIME || openFrom < now) && (openTo == DateTimeHelper.ZERO_TIME || now <= openTo) && (now <= eventDate))
            {
                ...
                if (allowRegistrationOverCapacity || (actualCount < capacity))
                {
                    ...
                    if (userInfo != null && auth && !RequestHelper.IsPostBack())
                    {
                ...
                if (EventNode != null)
                {
                    if (!EventAttendeeInfoProvider.IsRegisteredForEvent(EventNode.NodeID, txtEmail.Text.Trim()))
                    {
                        ...
                        if (eai != null)
                        {
                            ...
                            var activityInitializer = new EventBookingActivityInitializer(eai.AttendeeID, EventNode);
