...
using TimeZoneInfo = CMS.Globalization.Web.UI.TimeZoneUIMethods;
...
public partial class CMSWebParts_Events_OutlookEvent : CMSAbstractWebPart
{
    ...
    {
        if (StopProcessing || DocumentContext.CurrentDocument == null || DocumentContext.CurrentDocument.NodeClassName.ToLower() != "cms.bookingevent")
        {
            // Do nothing
            Visible = false;
        }
        else
        {
            ...
            eventDate = ValidationHelper.GetDateTime(EventNode.GetValue("EventDate"), DateTimeHelper.ZERO_TIME);
            openFrom = ValidationHelper.GetDateTime(EventNode.GetValue("EventOpenFrom"), DateTimeHelper.ZERO_TIME);
            openTo = ValidationHelper.GetDateTime(EventNode.GetValue("EventOpenTo"), DateTimeHelper.ZERO_TIME);
    ...
    protected void DisplayRegistration()
    {
        ...
        CurrentUserInfo userInfo = (CurrentUserInfo)MembershipContext.CurrentUserProfile;

        // Display registration form to anonymous user only if this is allowed
                if ((AllowAnonymousRegistration || (userInfo != null) && EventNode != null))
        {
            ...
            if ((openFrom == DateTimeHelper.ZERO_TIME || openFrom < now) && (openTo == DateTimeHelper.ZERO_TIME || now <= openTo) && (now <= eventDate))
            {
                ...
                if (allowRegistrationOverCapacity || (actualCount < capacity))
                {
                    ...
                    if (userInfo != null && !RequestHelper.IsPostBack())
                    {
    ...
    {
        string currentSiteName = CMS.SiteProvider.SiteContext.CurrentSiteName;
        // Check banned ip
                ...
                if (EventNode != null)
                {
                    if (!EventAttendeeInfoProvider.IsRegisteredForEvent(EventNode.NodeID, txtEmail.Text.Trim()))
                    {
                        ...
                        if (eai != null)
                        {
                            ...
                            CMS.Globalization.TimeZoneInfo tzi = null;
                            CMS.Globalization.Web.UI.TimeZoneUIMethods.GetDateTimeForControl(this, DateTime.Now, out tzi);
